...
export function calculateMetrics(data: FinancialData) {
  ...
  const booked = data.transactions.filter((transaction) => transaction.status === "booked" && !Boolean(transaction.isInternalTransfer));
  ...
  const pending = data.transactions.filter((transaction) => transaction.status === "pending").reduce((total, transaction) => total + Math.abs(transaction.amountCents), 0);
